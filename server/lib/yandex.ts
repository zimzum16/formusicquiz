...
async function fetchChart(): Promise<ChartEntry[]> {
  ...
  try {
    const resp = await fetch(`${YANDEX_API}/landing3?blocks=chart&lang=ru`, { headers: getHeaders() });
    if (!resp.ok) return [];
...
export async function getTrackInfo(title: string, artist: string): Promise<YandexTrackData> {
  ...
  try {
    ...
    if (!resp.ok) return { url: null, search_url, likes_count: null, play_count: null, chart: null };
    ...
    console.log('[yandex] track:', JSON.stringify(track, null, 2));
    ...
    if (likes_count === null && trackId) {
      ...
      if (detailResp.ok) {
        ...
        console.log('[yandex] detail:', JSON.stringify(detail, null, 2));
    ...
    return { url, search_url, likes_count, play_count, chart };
  } catch {
    return { url: null, search_url, likes_count: null, play_count: null, chart: null };
  }
}
